...
export const generateClothingPrompt = (productTitle, angleVariation, locationPrompt = '', hasLocationImage = false, seed = 0) => `
${ASPECT_RATIO_HEADER}
...
export const generateItemPrompt = (product, angleVariation, locationPrompt, seed, angleIndex = 0, hasLocationImage = false) => {
...
const detectSmartCategory = (title, category) => {
    const text = ((title || '') + ' ' + (category || '')).toLowerCase();

    if (text.includes('skincare') || text.includes('beauty') || text.includes('cream') || text.includes('serum') || text.includes('cosmet') || text.includes('lotion')) return 'skincare';
    if (text.includes('drink') || text.includes('bottle') || text.includes('glass') || text.includes('wine') || text.includes('liquid') || text.includes('beverag')) return 'drinks';
    if (text.includes('watch') || text.includes('rolex') || text.includes('jewelry') || text.includes('ring') || text.includes('necklace') || text.includes('earring') || text.includes('gold') || text.includes('diamond')) return 'jewelry';
    if (text.includes('food') || text.includes('snack') || text.includes('cook') || text.includes('fruit') || text.includes('burger')) return 'food';
    if (text.includes('phone') || text.includes('laptop') || text.includes('tech') || text.includes('headphone') || text.includes('gadget') || text.includes('electronic')) return 'tech';
...
export const generateStillLifePrompt = (product, angleVariation, locationPrompt, seed, hasLocationImage = false) => {
    return `
${ASPECT_RATIO_HEADER}
[TASK: Professional Still Life Product Placement - EXTREME CLOSE-UP]
- OBJECTIVE: Composite the "${product.title}" from Image 1 (Product) onto the background from Image 2 (Scene).
- CAMERA PERSPECTIVE: ${angleVariation} (MANDATORY: Render the product from this exact angle).
- FRAMING: Use an EXTREME CLOSE-UP shot. The product is the absolute HERO.
- SCALE: The product MUST fill the frame, occupying 80-90% of the vertical space. Minimize negative space.
- PLACEMENT RULE: PLACE the product from Image 1 DIRECTLY ONTO the top surface of the pedestal, shelf, or platform shown in Image 2.
- GEOMETRY & PHYSICS: 
    1. GROUNDING: The product must SIT FIRMLY on the surface. No floating.
    2. ZOOM: Drastically zoom in on the product to FILL the frame.
    3. EDGE-TO-EDGE: The pedestal and background MUST touch the left and right edges of the frame. 
    4. NO PADDING: No empty black spaces or margins allowed.
    5. CONTACT SHADOWS: Create realistic shadow contact points between product and surface.
    6. PERSPECTIVE: Match the camera angle defined in CAMERA PERSPECTIVE.
- NO HUMANS: ZERO presence of people, hands, or skin.
- BACKGROUND: ${hasLocationImage ? 'Image 2 is the MANDATORY background setup.' : (locationPrompt || 'Minimalist studio pedestal')}
- QUALITY: Photorealistic 8k, extreme detail, 9:16 vertical. [SEED: ${seed}]
`.trim();
};
